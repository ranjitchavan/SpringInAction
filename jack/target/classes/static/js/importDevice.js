...
function fillCustomerCombo() {
	...
	for (var i = 0; i < customerArray.length; i++) {
		customer += "<option value='" + customerArray[i].customerId + "'>"
...
function importDevice() {
	...
	const
	customerId = $('#SelectCustomer').find(":selected").text();

	const
	signatureFile = $('#file').prop('files')[0];

	if (customerId != null && customerId != "" && customerId != "Please Select") {
		form = new FormData();
		form.append("file", files[0]);
		form.append("SelectCustomer", customerId);
		...
		$
				.ajax({
					...
					success : function(data, statusText, xhr) {
						...
						if (xhr.status == 200) {
							if(data.length >=3){
								var message = "Import Completed. Total records :" + data[2] +",  Success: "+data[0] +",  Failure :"+
								data[1];
								fileName = data[3];
							}else{
								var message = 
...
function viewUnImportDevice(e) {
	...
	console.log("filename send :" +fileName);
	e.href = "";
